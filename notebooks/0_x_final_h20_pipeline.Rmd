...
calculate_prediction <- function(req) {
  ...
  input_df = fromJSON(req$postBody) %>% data_pipeline()
...
```{r}
fromJSON(test_sample_json) %>% data_pipeline() %>% toJSON
```

```{r}
fromJSON(test_sample_json) %>% data_pipeline() %>% toJSON %>% 
  write_json(path = "../data/interim/test_jason")
```

```{bash}
head ../data/interim/test_jason
```


```{bash}
curl -H "Content-Type: application/json" -X POST -d /home/stefan/non_packrat/Analytics_template/data/interim/test_jason http://127.0.0.1:8000/predict
```
